...
| G2 | Mane Style | V4 Regular (+$0), V5 Punk (+$3, **deactivated** July 2026 pending punk renders), V28 Long (+$3, **LIVE July 2026 for Ruby Red / Oxford White / Desert Sand only** — active:true, gated per-color; other colors hide the option until their sets land) |
...
- G2 V28 "Long" is a real product style (+$3), **partially LIVE (July 2026): active for Ruby Red (V1), Oxford White (V16), Desert Sand (V20) only** — the three colors with complete, head-aligned render sets. **Per-color gating** is the mechanism (so unsupported colors never 404): (a) `mane.ts` `MANE_STYLE_MAP` V28→"long"; (b) `mane.ts` exports `LONG_SUPPORTED_G1 = {V1,V16,V20}`, and `getManeContext` now reads G1 and falls back style→"reg" if Long is selected on an unsupported color (protects builder/cart/saved/share previews); (c) `OptionGroup.tsx` gained a `hiddenOptionIds` prop; (d) `build/[productSlug]/page.tsx` passes `hiddenOptionIds=["V28"]` to G2 when the selected color isn't in `LONG_SUPPORTED_G1`, and its onChange resets G2→V4 (Regular) if the user switches to an unsupported color while Long is selected (keeps preview + price consistent). **To add another color to Long:** finish its 8 head-aligned renders, then add its G1 id to `LONG_SUPPORTED_G1`. **Render progress (50 of 88):** Ruby/Oxford/Desert complete (8 each, LIVE); Velocity Blue / Shadow Black / Carbonized Gray — black only (4 each); Cactus Gray — black complete + white 3/4 (missing left); Azure Gray — white complete + black 3/4 (missing front = un-uploaded `Untitled_65`). All 50 head-aligned to regmane. **Still needed to expand Long:** white manes for Velocity/Shadow/Carbonized (12); Cactus white *left* (1); Azure black *front* (1); Eruption Green, Cyber Orange, Robin's Egg Blue (24). Full set = 88 (11 colors × 4 views × 2 mane colors).
...
- Mane style images: Punk (no renders yet); Long (V28 — **50 of 88 done**, all head-aligned to regmane: Ruby Red / Oxford White / Desert Sand complete; Velocity Blue + Shadow Black + Carbonized Gray black-only; Cactus Gray missing white-left; Azure Gray missing black-front; still need those white manes, cactus white-left, azure black-front, and Eruption Green / Cyber Orange / Robin's Egg Blue untouched; plus 3 provisional-slug colors brown/tan/sky_blue pushed but not G1-wired) — builder UI exists, layers disabled until full sets arrive
