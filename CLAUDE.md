...
| G2 | Mane Style | V4 Regular (+$0), V5 Punk (+$3, **deactivated** July 2026 pending punk renders), V28 Long (+$3, **deactivated** July 2026 — 39 of 88 renders done, pending full render set) |
...
- G2 V28 "Long" is a real product style (+$3), **deactivated** July 2026 pending its full render set. **Done (39 of 88):** Ruby Red, Oxford White, Desert Sand — **complete** (black + white, 8 each); Velocity Blue / Shadow Black — black mane only (4 each); Cactus Gray — black complete (4) + white 3/4 (**missing left**). **Still needed before relaunch:** white manes for Velocity Blue + Shadow Black (8); Cactus Gray white *left* (1); all of Eruption Green, Cyber Orange, Carbonized Gray, Azure Gray, Robin's Egg Blue (5 colors × 8 = 40). To relaunch: flip active:true and update MANE_STYLE_MAP V28→"long". Full set = 88 (11 colors × 4 views × 2 mane colors).
- **Held / unidentified color (July 2026):** two batches of Long renders arrived for bodies that don't match any of the 11 configured colors, held pending Todd's ID: (1) a **charcoal/dark gray** (`Untitled_9..12`, first batch); (2) a **saddle/chocolate brown**, plinth RGB ≈ (89,54,35), 8 renders (`Untitled_53..60`, both mane colors, all 4 views) — nearest config color is 44 RGB units away, so it is genuinely not in G1. Both need Todd to confirm the Ford color name/code + G1 slot (like the Robin's Egg Blue add) before processing/pushing. Identify uploaded colors by sampling the **plinth median RGB** and matching against reference `body_{color}_front_regmane_black.png` plinths.
...
- Mane style images: Punk (no renders yet); Long (V28 — **39 of 88 done**: Ruby Red / Oxford White / Desert Sand complete, Velocity Blue + Shadow Black black-only, Cactus Gray missing white-left; still need blue+black white manes, cactus white-left, 5 untouched colors, plus 2 held unidentified colors) — builder UI exists, layers disabled until full sets arrive
