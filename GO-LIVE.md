...
- [ ] **Finish Stripe Tax setup — the code is done, the dashboard is not.** Stripe Tax is integrated and deployed **inert**: checkout calls the Tax Calculation API and links the result to the PaymentIntent, but with no tax registration on file the calc returns $0, so every order currently records **Tax $0.00** (now always shown on the `/orders` list + detail pages). No code change or deploy is needed to turn it on — it activates automatically once the account is registered. Complete these in **Stripe Dashboard → Tax → Settings**:
  ...
  - [ ] **Confirm Pleeter LLC is actually registered to *collect* NJ Sales & Use Tax — prerequisite to the Stripe registration below.** The Certificate of Formation and Business Registration Certificate on file do **not** establish this; sales-tax collection requires having checked the sales-tax box on the NJ-REG and received a **Certificate of Authority (Form CA-1)**. As of Sep 2026 this is **unconfirmed** — and the formation cert lists the business purpose as "mobile computer applications," so retail sales tax may never have been set up. Call the NJ registration hotline **609-292-9292** (or check business.nj.gov) to confirm; if not registered, file the NJ-REG sales-tax addition first. State tax ID is **451-644-211** (the `/000` is a location suffix). Also update the address on file with NJ — it still shows the old Riverdale address, not 8 Nelke Ct, Hawthorne.
